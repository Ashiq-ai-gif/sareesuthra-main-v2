-- Initial schema for Saree Sutra e-commerce app (migrated from Supabase to InsForge)
...
-- Admin CRUD runs client-side with the anon key, matching the original project intent.
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='blogs' and policyname='Public can view blogs') then
    create policy "Public can view blogs" on public.blogs for select using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='blogs' and policyname='Admins can insert blogs') then
    create policy "Admins can insert blogs" on public.blogs for insert with check (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='blogs' and policyname='Admins can update blogs') then
    create policy "Admins can update blogs" on public.blogs for update using (true);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='blogs' and policyname='Admins can delete blogs') then
...
-- InsForge stores display metadata in auth.users.profile (jsonb).
...
begin
  ...
  values (new.id, new.email, coalesce(new.profile->>'name', ''))
...
do $$
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='addresses' and policyname='Users can view their own addresses.') then
    create policy "Users can view their own addresses." on public.addresses for select using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='addresses' and policyname='Users can insert their own addresses.') then
    create policy "Users can insert their own addresses." on public.addresses for insert with check (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='addresses' and policyname='Users can update their own addresses.') then
    create policy "Users can update their own addresses." on public.addresses for update using (auth.uid() = user_id);
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='addresses' and policyname='Users can delete their own addresses.') then
...
-- Admin panel + guest checkout use the client SDK with the anon key.
